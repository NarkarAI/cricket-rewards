...
const TABS: TabItem[] = [
  ...
  { label: "Players", href: "/players", icon: PlayersIcon, matchPaths: ["/players", "/reward"] },
  { label: "Dashboard", href: "/dashboard", icon: DashboardIcon, matchPaths: ["/dashboard", "/rewards"], requiresAuth: true },
  { label: "Wallet", href: "/wallet", icon: WalletIcon, matchPaths: ["/wallet"], requiresAuth: true },
  { label: "More", href: "#more", icon: ({ active }: { active: boolean }) => <MoreIcon />, matchPaths: [] },
];
...
export function BottomNav() {
  ...
  const isActive = (tab: TabItem): boolean => {
    ...
    return tab.matchPaths.some((p) => p !== "/" && pathname.startsWith(p));
  };
  ...
  const visibleTabs = user ? TABS : TABS.filter((t) => !t.requiresAuth);
        ...
        <div className="fixed left-2 right-2 z-50 bg-white border rounded-2xl shadow-lg px-2 py-3 space-y-1 md:hidden"
          ...
          {user ? (
            <>
              ...
              {user.role === "player" && (
                <Link href="/kyc" onClick={() => setMoreOpen(false)}
                  className="flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-gray-50 text-gray-700">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
